...
class PPOTrainer(ABC):
    ...
    def _compute_old_log_prob(self, batch: KVBatchMeta, metrics: dict) -> KVBatchMeta:
        ...
        # Entropy is only needed for explicit monitoring or entropy regularization.
        # Avoid materializing a second full-vocabulary tensor when it is disabled.
        ...
        # 2. write response-only old_log_probs and optional entropy back to TransferQueue
        ...
        # 3. calculate actor entropy metrics only when explicitly requested
        if calculate_entropy:
    ...
    def _compute_advantage(self, batch: KVBatchMeta, metrics: dict) -> KVBatchMeta:
        ...
        difficulty_grpo_enabled = self.config.algorithm.adv_estimator in {
            "maptab_hybrid_dgpo",
            "maptab_xuezhang_grpo",
        }
        hybrid_dgpo_enabled = self.config.algorithm.adv_estimator == "maptab_hybrid_dgpo"
        xuezhang_grpo_enabled = self.config.algorithm.adv_estimator == "maptab_xuezhang_grpo"
        if difficulty_grpo_enabled:
            fields.append("extra_fields")
        data = tq.kv_batch_get(keys=batch.keys, partition_id=batch.partition_id, select_fields=fields)

        response_mask = data["response_mask"]
        accuracy_rewards = None
        image_difficulties = None
        xuezhang_difficulty_weights = None
        ...
        if difficulty_grpo_enabled:
            ...
            if hybrid_dgpo_enabled:
                image_difficulties = []
            if xuezhang_grpo_enabled:
                xuezhang_difficulty_weights = []
            for extra_fields in data.pop("extra_fields").tolist():
                extra_fields = getattr(extra_fields, "data", extra_fields)
                reward_info = extra_fields.get("reward_extra_info", {}) if isinstance(extra_fields, dict) else {}
                if "accuracy_reward" not in reward_info:
                    raise RuntimeError("Difficulty-weighted GRPO requires accuracy_reward in reward_extra_info")
                accuracy_rewards.append(float(reward_info["accuracy_reward"]))
                reward_components["part_acc_rewards"].append(
                    float(reward_info.get("part_acc_reward", reward_info["accuracy_reward"]))
                )
                reward_components["format_rewards"].append(float(reward_info.get("format_reward", 0.0)))
                reward_components["overlong_rewards"].append(float(reward_info.get("overlong_reward", 0.0)))
                if hybrid_dgpo_enabled:
                    if "image_difficulty" not in reward_info:
                        raise RuntimeError("Hybrid DGPO requires image_difficulty in reward_extra_info")
                    image_difficulties.append(float(reward_info["image_difficulty"]))
                if xuezhang_grpo_enabled:
                    if "xuezhang_difficulty_weight" not in reward_info:
                        raise RuntimeError(
                            "Xuezhang GRPO requires xuezhang_difficulty_weight in reward_extra_info"
                        )
                    xuezhang_difficulty_weights.append(
                        float(reward_info["xuezhang_difficulty_weight"])
                    )
        ...
        if difficulty_grpo_enabled:
            data.batch["accuracy_rewards"] = torch.tensor(
                accuracy_rewards,
                dtype=data.batch["token_level_scores"].dtype,
                device=data.batch["token_level_scores"].device,
            )
            for key, values in reward_components.items():
                data.batch[key] = torch.tensor(
                    values,
                    dtype=data.batch["token_level_scores"].dtype,
                    device=data.batch["token_level_scores"].device,
                )
        if hybrid_dgpo_enabled:
            data.batch["image_difficulties"] = torch.tensor(
                image_difficulties,
                dtype=data.batch["token_level_scores"].dtype,
                device=data.batch["token_level_scores"].device,
            )
        if xuezhang_grpo_enabled:
            data.batch["xuezhang_difficulty_weights"] = torch.tensor(
                xuezhang_difficulty_weights,
                dtype=data.batch["token_level_scores"].dtype,
                device=data.batch["token_level_scores"].device,
            )
        ...
        if hybrid_dgpo_enabled:
            prefix = "training/hybrid_dgpo"
            metrics[f"{prefix}/pass_rate_mean"] = data.batch["hybrid_pass_rate"].mean().item()
            metrics[f"{prefix}/image_difficulty_mean"] = data.batch["image_difficulties"].mean().item()
            metrics[f"{prefix}/difficulty_mean"] = data.batch["hybrid_difficulty"].mean().item()
            question_weights = data.batch["hybrid_question_weight"]
            positive_weights = question_weights[question_weights > 0]
            if positive_weights.numel():
                metrics[f"{prefix}/weight_mean"] = positive_weights.mean().item()
                metrics[f"{prefix}/weight_min"] = positive_weights.min().item()
                metrics[f"{prefix}/weight_max"] = positive_weights.max().item()
            for name in ("final", "exact", "partial", "format", "overlong"):
                key = f"hybrid_{name}_variance"
                if key in data.batch:
                    metrics[f"{prefix}/{name}_variance_group_fraction"] = data.batch[key].mean().item()
        if xuezhang_grpo_enabled:
            prefix = "training/xuezhang_grpo"
            metrics[f"{prefix}/pass_rate_mean"] = data.batch["xuezhang_pass_rate"].mean().item()
            question_weights = data.batch["xuezhang_question_weight"]
            positive_weights = question_weights[question_weights > 0]
            if positive_weights.numel():
                metrics[f"{prefix}/weight_mean"] = positive_weights.mean().item()
                metrics[f"{prefix}/weight_min"] = positive_weights.min().item()
