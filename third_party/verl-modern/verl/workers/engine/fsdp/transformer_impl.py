...
def _patch_fsdp2_accumulated_grad_guard() -> None:
    """Skip grad accumulation for FSDP2 params never materialized in forward."""
    from torch.distributed.fsdp._fully_shard._fsdp_param import FSDPParam

    current = FSDPParam.to_accumulated_grad_if_needed
    if getattr(current, "_verl_missing_unsharded_guard", False):
        return

    def guarded_to_accumulated_grad_if_needed(fsdp_param):
        # PyTorch's final reduce branch already has this guard, but its
        # no-sync/gradient-accumulation branch does not in torch 2.11.
        if not hasattr(fsdp_param, "_unsharded_param"):
            return
        return current(fsdp_param)

    guarded_to_accumulated_grad_if_needed._verl_missing_unsharded_guard = True
    FSDPParam.to_accumulated_grad_if_needed = guarded_to_accumulated_grad_if_needed
...
class FSDPEngine(BaseEngine):
    ...
    def _build_lora_module(self, module):
        ...
        trainable_pattern = getattr(self.model_config, "lora_trainable_modules", None)
        if trainable_pattern:
            pattern = re.compile(str(trainable_pattern))
            adapter_count = 0
            trainable_count = 0
            trainable_numel = 0
            frozen_numel = 0
            ...
            for name, param in module.named_parameters():
                if "lora_" not in name:
                    continue
                adapter_count += 1
                trainable = pattern.fullmatch(name) is not None
            ...
            if adapter_count == 0 or trainable_count == 0:
                raise ValueError(
                    "lora_trainable_modules did not select any LoRA tensors: "
                    f"pattern={trainable_pattern!r}, adapter_tensors={adapter_count}"
                )
            logger.warning(
                "Composite LoRA partition: trainable=%d/%d tensors "
                "(%d parameters), frozen=%d parameters, examples=%s",
                trainable_count,
                adapter_count,
                trainable_numel,
                frozen_numel,
    ...
    def _build_fsdp_module(self, module):
        ...
        if self.engine_config.strategy == "fsdp":
            ...
            _patch_fsdp2_accumulated_grad_guard()
    ...
    def prepare_model_inputs(self, micro_batch: TensorDict):
        ...
        if use_fused_kernels:
            ...
            if model_type in {"qwen3_5", "qwen3_5_moe"}:
                extra_args["calculate_entropy"] = tu.get_non_tensor_data(
