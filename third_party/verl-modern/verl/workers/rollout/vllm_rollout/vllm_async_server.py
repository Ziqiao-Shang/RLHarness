...
class vLLMHttpServer:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None, dp_rpc_port: int = None):
        ...
        startup_stagger_seconds = float(engine_kwargs.pop("startup_stagger_seconds", 0.0))
        ...
        # 3. launch server
        if self.node_rank == 0:
            # vLLM's multiprocess executor probes and then releases an ephemeral
            # port before TCPStore binds it. Stagger colocated replicas to avoid
            # a cross-process EADDRINUSE race during concurrent startup.
            await asyncio.sleep(self.replica_rank * startup_stagger_seconds)
