        ...
        for key, tensor in batch.items():
            if isinstance(tensor, torch.Tensor) and not tensor.is_nested:
                ...
                try:
                    ...
                    # PyTorch can unbind 3D+ jagged tensors along the wrong
                    # values dimension (notably mRoPE [batch, 4, seq]).
                    if "split_with_sizes expects split_sizes" not in str(exc):
                        raise
                    offsets = tensor.offsets()
                    lengths = offsets.diff().tolist()
                    total_length = int(offsets[-1].item())
                    values = tensor.values()
                    candidate_dims = [dim for dim, size in enumerate(values.shape) if size == total_length]
                    if not candidate_dims:
                        raise RuntimeError(
                            f"Cannot recover nested tensor {key!r}: no values dimension "
                            f"matches offsets total {total_length}; values shape is {tuple(values.shape)}"
                        ) from exc
                    ragged_idx = getattr(tensor, "_ragged_idx", tensor.dim() - 1)
                    expected_dim = ragged_idx - 1
                    split_dim = expected_dim if expected_dim in candidate_dims else candidate_dims[0]
                    tensor_lst = torch.split(values, lengths, dim=split_dim)
