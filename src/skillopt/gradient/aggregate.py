...
def _merge_batch(
    ...
    fallback_reason = "optimizer returned invalid JSON or omitted the required payload"
    try:
    ...
    for p in patches:
        for e in get_payload_items(p, update_mode):
            ...
            all_edits.append(e)
    return {
        "reasoning": f"fallback concatenation after merge failure: {fallback_reason}",
        payload_key(update_mode): all_edits,
    }
