...
from skillopt.model import chat_optimizer, chat_optimizer_messages
...
def fmt_minibatch_trajectories(
    ...
    for idx, item in enumerate(items, 1):
        ...
        optimizer_context = item.get("optimizer_context", "")
        if not optimizer_context:
            optimizer_context_path = os.path.join(prediction_dir, tid, "optimizer_context.txt")
            if os.path.exists(optimizer_context_path):
                with open(optimizer_context_path) as f:
                    optimizer_context = f.read()
        user_prompt = ""
        if not optimizer_context:
        ...
        if optimizer_context:
            header += (
                f"\n#### Target Instance Context (static S0 shown once above)\n"
                f"{optimizer_context}\n"
            )
        elif user_prompt:
            header += (
...
def _optimizer_call(
    *,
    ...
    """Call the optimizer with trajectory images when the corpus provides them."""
    image_cases: dict[str, list[str]] = {}
    if include_images:
        for item in items:
            ...
            for raw_path in paths:
                ...
                if not Path(path).is_file():
                    raise FileNotFoundError(
                        f"trajectory {case_id} declares a missing optimizer image: {path}"
                    )
                image_cases.setdefault(path, []).append(case_id)

    if not image_cases:
        return chat_optimizer(
            system=system,
            user=user,
    ...
    for path, case_ids in image_cases.items():
        mime = mimetypes.guess_type(path)[0] or "image/png"
        encoded = base64.b64encode(Path(path).read_bytes()).decode("ascii")
        content.append(
            {
                "type": "text",
                "text": "Attached map image for trajectory IDs: " + ", ".join(case_ids),
            }
        )
        content.append(
            {
    ...
    return chat_optimizer_messages(
        messages=[
