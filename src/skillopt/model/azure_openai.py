...
def _chat_impl(
    ...
    raise RuntimeError(f"LLM call failed after {retries} retries: {type(last_err).__name__}") from None
...
def _chat_messages_impl(
    ...
    *,
    ...
    for attempt in range(retries):
        try:
            if _needs_responses_api(deployment):
                ...
                elif actual_effort is not None and not deployment.lower().startswith("gemini"):
                    kwargs["reasoning_effort"] = actual_effort
                if timeout is not None:
                    kwargs["timeout"] = timeout
                resp = client.chat.completions.create(**kwargs)
    ...
    raise RuntimeError(f"LLM message call failed after {retries} retries: {type(last_err).__name__}") from None
