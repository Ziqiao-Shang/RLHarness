...
def _redact_cfg(cfg: dict) -> dict:
    ...
    for key in list(redacted):
        if (key.lower() in _SECRET_KEYS or "api_key" in key.lower()) and redacted.get(key):
...
class ReflACTTrainer:
    ...
    def train(self) -> dict:
        ...
        uncapped_total_steps = num_epochs * steps_per_epoch
        max_train_steps = int(cfg.get("max_train_steps", 0) or 0)
        total_steps = (
            min(uncapped_total_steps, max_train_steps)
            if max_train_steps > 0
            else uncapped_total_steps
        )
        ...
        if total_steps < uncapped_total_steps:
            print(
                f"  [config] capped training at {total_steps}/"
                f"{uncapped_total_steps} steps"
            )
        ...
        if runtime_state:
            ...
            current_score = float(runtime_state.get("current_score", -1.0))
            best_score = float(runtime_state.get("best_score", current_score))
        ...
        if current_score < 0:
            ...
            sel_cache[skill_init] = (baseline_hard, baseline_soft)
            current_origin = "initial_skill"
            best_origin = "initial_skill"
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for record in history:
                if record.get("epoch") == epoch and record["step"] < resume_from:
                    digest_path = os.path.join(
                        out_root, "steps", f"step_{record['step']:04d}", "trajectory_digest.json",
                    )
                    if os.path.exists(digest_path):
                        with open(digest_path) as f:
                            step_buffer.append(json.load(f))
            active_meta_skill = (
            ...
            for step_in_epoch in range(steps_per_epoch):
                ...
                candidate_validator = getattr(adapter, "validate_candidate_skill", None)
                if callable(candidate_validator):
                    invariant_ok, invariant_reasons = candidate_validator(candidate_skill)
                    invariant_report = {
                        "valid": bool(invariant_ok),
                        "reasons": list(invariant_reasons),
                    }
                    with open(os.path.join(step_dir, "prompt_invariants.json"), "w") as f:
                        json.dump(invariant_report, f, indent=2, ensure_ascii=False)
                    step_rec["prompt_invariants"] = invariant_report
                    if not invariant_ok:
                        step_rec["action"] = "reject_prompt_invariants"
                        step_rec["current_score"] = current_score
                        step_rec["best_score"] = best_score
                        step_rec["best_step"] = best_step
                        step_rec["skill_len"] = len(current_skill)
                        ...
                        with open(os.path.join(step_dir, "step_record.json"), "w") as f:
                            json.dump(step_rec, f, indent=2, ensure_ascii=False)
                        print(
                            "    [reject] candidate violated immutable prompt contract: "
                ...
                # ⑥ EVALUATE ───────────────────────────────────────────────
                t_phase = time.time()
                if candidate_skill in sel_cache:
                    cand_hard, cand_soft = sel_cache[candidate_skill]
                    print(
                        f"    [6/6 EVALUATE] "
                        f"cache hit: hard={cand_hard:.4f}"
                    )
                else:
                    sel_env, sel_n = _build_eval_env(
                        split="valid_seen",
                    ...
                    sel_cache[candidate_skill] = (cand_hard, cand_soft)
                ...
                if os.path.exists(slow_done_path):
                    ...
                    if slow_result and slow_result.get("slow_update_content"):
                        ...
                        if slow_gate_with_selection:
                            ...
                            if slow_candidate in sel_cache:
                                slow_sel_hard, slow_sel_soft = sel_cache[slow_candidate]
                                print(
                            ...
                            # Reuse the score when this exact Skill text recurs.
                            sel_cache[current_skill] = (current_score, 0.0)
